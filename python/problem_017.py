...
def count_letters(stop_number, letter_count_dict):
    ...
    for n in range(1,stop_number+1):
        ...
        elif (len(str(n)) == 2) | n > 20:
            sum_letters += sum([letter_count_dict[int(character+'0')] 
                                if i == 0 
                                else letter_count_dict[int(character)]
                                for i, character in enumerate(str(n))])    
        elif len(str(n)) == 3:
            pass
    return sum_letters
